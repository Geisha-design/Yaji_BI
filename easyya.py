...
def yoxi(url,babyId):
    ...
    if cc:
        ...
        oo = page.ele('@class=sku-selector-name',timeout=1)
        if oo:
            name = page.ele('@class=sku-selector-name',timeout=1).text
        guige = ''
        pps = ''
    ...
    if names:
        ...
        if len(names)>1:
            ...
            guigexinxi = page.eles('@class=sku-item-name',timeout=0.5)
            ...
            name = page.ele('@class=sku-prop-module-name',timeout=0.5).text
            guige = ""
            pps = ''
            print(name)

    else:
        names = page.eles("@class=sku-selector-name",timeout=0.5)
        if len(names)>1:
            print(names[0].text)
            print(names[1].text)
            name = names[0].text
            ...
            CC = page.ele('@class=sku-selector-name',timeout=0.5)
            if CC:
                name = page.ele('@class=sku-selector-name',timeout=0.5).text
                guige = ""
                pps = ''
                print(name)

    # cc = page.ele('sku-item-wrapper')
    cc = page.ele('@class=prop-name',timeout=0.5)
    if cc:

        values = page.eles('@class=prop-name',timeout=1)
    else:

        values = page.eles('@class=sku-item-wrapper',timeout=0.1)
        if values:
            values = page.eles('@class=sku-item-wrapper', timeout=0.1)
        else:
            pp = page.ele('@class=item-label',timeout=0.5)
            if pp:
                values = page.eles('@class=item-label',timeout=0.1)
            else:
                values = page.eles('@class=prop-item-text',timeout=0.5)
...
# //*[@id="sku-count-widget-wrapper"]/div[1]
def convert():
...
if __name__ == "__main__":
    ...
    for batch in fetch_company_names_in_batches_url2(batch_size=10,start_offset=start_offset):
        if start_offset >= end_offset:  # 检查是否超出范围
            print("已达到结束偏移量，停止处理。")
            break
        print(f"正在处理批次：{batch}，{start_offset}")
        print(start_offset)
        for babyId,contact_url,id,company_name in batch:
            print(f"正在更新公司：{babyId},{contact_url},{id},{company_name},{start_offset}")
            # 这里插入 RPA自定义的代码，用于获取公司信息并更新
            try:
                # 适当的加载时间是必须的 这样才能确保不会被干掉 之前是4秒
                time.sleep(2)
